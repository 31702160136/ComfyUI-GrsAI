...
支持 7 种主流模型，价格低至￥0.02/次。

## 🚀 概述

ComfyUI-GrsAI 是一个专业的 ComfyUI 扩展，提供完整的多模型图像生成功能。支持 7 种主流AI模型，经过深度优化，为用户提供最简洁的配置体验和最强大的功能支持。
...
- 🌟 计划支持 `nano-banana-pro` 模型，并加入专属参数 `imageSize`（1K/2K/4K，默认 1K）
- 🧭 新建 `feature/nano-banana-pro` 分支用于开发，后续版本将随功能落地发布
...
- 🌟 **用户体验** - 中文界面，友好反馈
- 🛡️ **稳定性** - 完善错误处理和重试机制
- 💰 **多模型支持** - 7种模型（含 nano-banana-pro），价格从￥0.02起
